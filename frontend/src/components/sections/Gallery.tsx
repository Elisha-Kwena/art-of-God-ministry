      ...
      <section className="w-full py-12 welcome">
        <div className="w-[90%] mx-auto flex flex-col items-center justify-center gap-4">
          <div className="w-full">
            <h1 className="text-6xl text-center mb-4 font-bold text-[#ab4f43]">Visualizing Our Faith Journey</h1>
            <p className="text-xl text-black text-center">Through the Lens of Faith: Moments That Move Us</p>
            <p className="text-xl text-black text-center">A living scrapbook of worship, service, and fellowship—where God's love becomes visible.</p>
          </div>
          <p className="text-xl text-black text-center">CLICK TO VIEW FULL PHOTO</p>
          <div className="w-full h-[90vh] py-4 flex items-center justify-between gap-2">
            {/* left */}
            <div className="w-1/4 h-full flex flex-col gap-2">
              <div className="h-1/3 w-full bg-red rounded-md cursor-pointer" onClick={() => openModal(images.left1)}>
                <img src={left1} alt={images.left1.alt} className="rounded-md w-full h-full object-cover"/>
              </div>
              <div className="h-2/3 w-full bg-red rounded-md cursor-pointer" onClick={() => openModal(images.left2)}>
            ...
            <div className="w-2/4 h-full flex flex-col gap-2">
              <div className="w-full bg-red h-[60%] rounded-md cursor-pointer" onClick={() => openModal(images.middle1)}>
              ...
              <div className="w-full h-[40%] flex items-center justify-between gap-2">
                <div className="h-full rounded-md bg-red w-[40%] cursor-pointer" onClick={() => openModal(images.bottom1)}>
                  <img src={bottom1} alt={images.bottom1.alt} className="rounded-md w-full h-full object-cover"/>
                </div>
                <div className="h-full rounded-md bg-red w-[60%] cursor-pointer" onClick={() => openModal(images.bottom2)}>
            ...
            {/* right */}
            <div className="w-1/4 h-full flex flex-col gap-2">
              <div className="h-2/3 w-full bg-red rounded-md cursor-pointer" onClick={() => openModal(images.right1)}>
                <img src={right1} alt={images.right1.alt} className="rounded-md w-full h-full object-cover"/>
              </div>
              <div className="h-1/3 w-full bg-red rounded-md cursor-pointer" onClick={() => openModal(images.right2)}>
                <img src={right2} alt={images.right2.alt} className="rounded-md w-full h-full object-cover"/>
              </div> 
            </div>
          </div>
          <div className="mt-16 flex flex-col items-center justify-center">
            <p className="mb-8 text-xl">Share your photos with us @ChurchName #OurFaithFamily</p>
            <Link to="" className="uppercase text-3xl text-[#ab4f43] border-[2px] border-[#ab4f43] p-2 px-5 rounded-[3px] transition-all duration-300 hover:text-white hover:bg-[#ab4f43]">View more in our Gallery
            </Link>
          </div>
        </div>
      </section>
